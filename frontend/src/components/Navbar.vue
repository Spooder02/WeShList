<template>
<nav class="block shadow-lg mb-4">
    ...
    <div class="w-full block flex-grow flex items-center justify-between p-2">
      <div class="flex items-center" @click="$router.push('/')" >
        <img src="@/assets/WeShList_Eng-Logo.png" class="apect-w-5 aspect-h-1 h-8 ml-1 mr-1">
        <h1 class="text-xl font-bold text-black pt-[0.25em]">WeShList</h1>
        </img>
      </div>
        <button @click="toggleMenu()" class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-50 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
        <svg v-if="!isOpen" class="block h-8 w-8" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="black" aria-hidden="true">
    ...
    <div v-if="isOpen" class="w-full bg-gray-50 p-4 text-center text-black">
      <div v-if="isLoggedin" class="mb-2">
        <span class="font-semibold">Welcome, {{ username }}!</span>
        <a class="ml-4" @click="logout()">Logout</a>
      </div>
      <a @click="$router.push('/')" class="block">Main</a>
      <a @click="$router.push('/introduct')" class="block mt-2">Introduction</a>
      <a @click="$router.push('/additem')" class="block mt-2">Report Product</a>
      <a @click="$router.push('/finditem')" class="block mt-2">Find Product</a>
      <div v-if="!isLoggedin" class="mt-3">
        <a @click="$router.push('/login')" class="font-medium">Sign in</a>
        <span class="ml-1 mr-1">|</span>
        <a @click="$router.push('/signup')" class="font-medium">Sign up</a>
      </div>
    </div>
</nav>
</template>
